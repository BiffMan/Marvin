...
public class TabWidget extends GridWidget
{
    ...
    public boolean Create(TabPane tabPane, DataManager dataMgr, int iIndex)
    {
        ...
        if (true == fSuccess)
        {
            ...
            StackPane stackReference = new StackPane(); // for back filler when translating
//            stackReference.setStyle("-fx-background-color:red");
            ...
            if (_UseScrollBars)
            {
                ...
                new TranslationCalculator(stackReference, _BaseGridPane, CONFIG.getScaleProperty(), getPosition()); // handles all the resizing/scaling
            }
            else
            {
                basePane.getChildren().add(stackReference);
