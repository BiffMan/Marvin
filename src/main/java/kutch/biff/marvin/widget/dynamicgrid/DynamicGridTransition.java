 ...
 */
public class DynamicGridTransition
{
    ...
    private DynamicGridTransition.Transition _Type;
    ...
    public DynamicGridTransition(DynamicGridTransition.Transition which)
    {
        _Type = which;
        playing = false;
        _HideBackSize = true; // need this for images (like of widgets) with transparent parts
        //_Type = DynamicGridTransition.Transition.NONE;
    }
    ...
    private void Setup()
    {
        ...
        {
            case FLIP_HORIZONTAL:
                _Type = DynamicGridTransition.Transition.RECTANGULAR_AROUND_Y;
                noOfTilesX = 1;
                noOfTilesY = 1;
                break;
            case FLIP_VERTICAL:
                _Type = DynamicGridTransition.Transition.RECTANGULAR_AROUND_X;
                noOfTilesX = 1;
                noOfTilesY = 1;
                break;
        }
        ...
        int x = GridPane.getColumnIndex(fromGrid.getBasePane());
        int y = GridPane.getRowIndex(fromGrid.getBasePane());

        Parent.add(pane, x, y);
    }

    public void Transition(DynamicGrid fromGridObj, DynamicGrid toGridOb, GridPane objParent)
    {
        fromGrid = fromGridObj;
        toGrid = toGridOb;
        ...
        startImage = fromGrid.getImage(fromGridObj.getBackgroundColorForTransition());
        toGrid.getBasePane().setVisible(true); // needs to be visable for snapshot to work
        
        endImage = toGrid.getImage(toGridOb.getBackgroundColorForTransition());
        toGrid.getBasePane().setVisible(false);
        fromGrid.getBasePane().setVisible(false);

        stepSizeX = endImage.getWidth() / noOfTilesX;
        stepSizeY = endImage.getHeight() / noOfTilesY;
        
        if (null != pane)
        {
            double width,height;
            width = endImage.getWidth();
            height = endImage.getHeight();
        ...
        {
            case VERTICAL_AROUND_X:
                rotateVerticalTilesAroundX(startImage, endImage, interpolator, duration, delay);
                break;
            case VERTICAL_AROUND_Y:
                rotateVerticalTilesAroundY(startImage, endImage, interpolator, duration, delay);
                break;
            case VERTICAL_AROUND_X_AND_Y:
                rotateVerticalTilesAroundXandY(startImage, endImage, interpolator, duration, delay);
                break;
            case HORIZONTAL_AROUND_X:
                rotateHorizontalTilesAroundX(startImage, endImage, interpolator, duration, delay);
                break;
            case HORIZONTAL_AROUND_Y:
                rotateHorizontalTilesAroundY(startImage, endImage, interpolator, duration, delay);
                break;
            case HORIZONTAL_AROUND_X_AND_Y:
                rotateHorizontalTilesAroundXandY(startImage, endImage, interpolator, duration, delay);
                break;
            case RECTANGULAR_AROUND_X:
                rotateRectangularTilesAroundX(startImage, endImage, interpolator, duration, delay);
                break;
            case RECTANGULAR_AROUND_Y:
                rotateRectangularTilesAroundY(startImage, endImage, interpolator, duration, delay);
                break;
            case RECTANGULAR_AROUND_X_AND_Y:
                rotateRectangularTilesAroundXandY(startImage, endImage, interpolator, duration, delay);
                break;
            case DISSOLVING_BLOCKS:
                dissolvingBlocks(startImage, endImage, interpolator, duration, delay);
                break;
            case CUBE:
                cube(startImage, endImage, interpolator, duration, delay);
                break;
            case FLIP_HORIZONTAL:
                flipHorizontal(startImage, endImage, interpolator, duration, delay);
                break;
            case FLIP_VERTICAL:
                flipVertical(startImage, endImage, interpolator, duration, delay);
                break;
    ...
    /**
     * Rotate vertical tiles around x and y transition between front- and
     * backimage
     *
     * @param FRONT_IMAGE
     * @param BACK_IMAGE
     ...
     * @param DELAY delay in milliseconds between each tile animation
     */
    private void dissolvingBlocks(final Image FRONT_IMAGE, final Image BACK_IMAGE, final Interpolator INTERPOLATOR, final Duration DURATION, final int DELAY)
    {
        splitImageXY(FRONT_IMAGE, BACK_IMAGE);
    ...
    private void flipVertical(final Image FRONT_IMAGE, final Image BACK_IMAGE, final Interpolator INTERPOLATOR, final Duration DURATION, final int DELAY)
    {
        viewPorts.clear();
    ...
    private void checkVisibility(final Rotate ROTATE_X, final Rotate ROTATE_Y, final int INDEX)
    {
        if (_HideBackSize)
        {
            imageViewsBack.get(INDEX).setVisible(false);  // need this for images (like of widgets) with transparent parts
        }
    ...
    private void transitionFinished()
    {
        ...
        toGrid.getStylableObject().setVisible(true);
