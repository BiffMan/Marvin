...
import kutch.biff.marvin.utility.StringAsNumberComparator;
    ...
    private void Sort()
    {
        if (getSortMode() == SortMode.Ascending)
        {
            Collections.sort(_DataPoint, new Comparator<Pair<String, SteelLCDWidget>>()
            {
                ...
                {
                    StringAsNumberComparator naturalCompare = new StringAsNumberComparator();
