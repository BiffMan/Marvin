...
import kutch.biff.marvin.widget.dynamicgrid.DynamicGridTransition;
...
public class DynamicGridWidget extends GridWidget
{
    ...
    private DynamicGridTransition _latestTransition;
    ...
    public boolean Create(GridPane parentPane, DataManager dataMgr)
    {
        if (super.Create(parentPane, dataMgr))
        {
            ...
            if (null != _CurrentKey)
            {
                ...
                {
                    LOGGER.warning("Initial ID for Dynamic Grid: " + _CurrentKey + " is invalid.");
                    return false;
                }
    ...
    private void ActivateGrid(String key)
    {
        ...
        if (_GridMap.containsKey(key)) // specified grid ID is valid, so let's proceed
        {
            ...
            if (null != objGridNext)
            {
                ...
                else
                {
                    ...
                    _latestTransition = objGridNext.Transition(objGridCurrent, _TransitionPane);
                }
                _CurrentKey = key;
            }
        ...
        {
            LOGGER.warning("Received unknown ID for DynamicGrid: [" + getNamespace() + ":" + getMinionID() + "] : " + key);
            return;
        }
        if (_AutoAdvance)
        {
    ...
    public boolean HandleWidgetSpecificSettings(FrameworkNode node)
    {
        ...
        if (node.getNodeName().equalsIgnoreCase("GridFile"))
        {
            ...
            if (!objGrid.ReadTransitionInformation(node))
            {
                return false;
            }
